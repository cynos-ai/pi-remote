...
if (!["basic", "controls", "configuration", "defaults", "compact", "compact-cancel", "compact-cancel-stream"].includes(scenario) || (scenario !== "basic" && suite !== "commands")) {
...
function requireLiveConfiguration() {
  ...
  if (!Number.isSafeInteger(operations) || operations < (scenario.startsWith("compact") ? 8 : ["configuration", "defaults"].includes(scenario) ? 4 : scenario === "controls" ? 3 : suite === "sdk" ? 2 : 1) || !Number.isFinite(timeout) || timeout < 1000 || timeout > 1800000) return false;
