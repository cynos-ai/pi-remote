...
custom/editor 的 select 选项增加“组合键”，随后使用现有 input 收集如 `ctrl+alt+k` 的键名，并按原生终端序列送入输入监听与组件。两次响应各自消费独立 interactionId，重试不重复送键。非法键名使用既有扩展错误通知并返回 select；不新增事件、命令或 Interaction kind。普通“输入文本”不解析快捷键字符串，editor-state 草稿同步也不触发原始终端输入监听。

编辑器工厂复用 custom.render、setEditorText 和 select/input/respond；没有新核心事件或数据库迁移。例如 `{method:"custom.render",args:["editor-operation",["draft"]]}` 是对应待答控件的显示副本。setEditorComponent / getEditorComponent / addAutocompleteProvider 的函数不序列化。每个按键按既有 interactionId 幂等消费；respond completed 仅表示输入被接收，不表示提交产生的模型 Run 成功。onSubmit 产生独立操作及原生 Run，不把该 Run 绑定安装编辑器的旧 Command。旧交互取消后拒绝回答，重放画面不自动重新提交。
...
- 托管编辑器的思考块显示动作复用 `extension_ui`，发送 `{method:"setThinkingVisible",args:[boolean]}`；它是显示投影，不修改公共 Session DTO。SDK 的 hideThinkingBlock 设置先保存，再发布通知；快照重放恢复手机显示。redacted thinking 始终只显示隐藏标签。
...
- custom message 的 `custom.renderer?` 为 `{lines:string[],expanded:boolean,width:80,truncated?,failed?}`，只保存 worker 执行原生 renderer 后的纯文本显示投影；模型上下文仍来自 blocks。renderer 缺失、返回 undefined、异常或 `display:false` 不伪造成功，按 SDK 使用原消息回退或隐藏。`custom_entry.appended` 使用同一 renderer DTO，payload 为 `{entryId,type,renderer}`，需要 operationId、runId 可空，reducer 立即封存为 `timeline_items.kind=custom_entry`；它不进入模型上下文。最多 256 行 / 32768 字并标记截断，函数、SDK Component 和扩展回调不进入公共协议。
