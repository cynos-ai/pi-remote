...
| FR08 | 执行、初始化、配置及扩展回调中的 select / confirm / input / editor / image 请求、手机回答和重连恢复 |
...
| 应用监听与扩展快捷键 | onTerminalInput 绑定同 Session 原生交互表面，先消费/改写再路由；CustomEditor 沿 SDK 冲突规则匹配扩展快捷键，保留自定义覆盖；组合键走一次性表单；默认应用动作另验 |
