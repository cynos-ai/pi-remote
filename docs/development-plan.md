...
编辑器适配复用持久表单送入原生输入路由，验证安装返回、草稿同步及光标保留、补全包装、提交与独立 Run/无 Run Bash、替换/取消和旧回调失效。终端菜单、全局快捷键和压缩队列对照保持单独缺口，不通过隐藏入口或误发模型文本绕过。

应用级输入增量覆盖 onTerminalInput 的原生顺序/消费/转换、晚注册/取消、同 Session 表面绑定及原生替换清理；扩展快捷键覆盖 getShortcuts 冲突规则、CustomEditor 自定义覆盖、异步回调异常和无模型副作用。组合键编码以固定版本原生 matcher 验证，SDK 的修饰功能键限制独立记录；默认应用动作仍另行接入与验收。
