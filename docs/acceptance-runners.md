...
| `pnpm test:live -- --suite commands` | 默认：真实工具往返、同键请求、空闲配置；`--scenario controls`：stop 草稿、steer/follow-up、旧 target 与执行顺序 | streaming 配置、compact 队列、全阶段表单、原生会话/标题、自主与多 Run，以及带附件的 stop 对照 |
...
| `PI_REMOTE_LIVE_MAX_OPERATIONS` | 明确允许的顶层模型操作数；sdk 至少 2，commands/realtime 基础场景至少 1，controls 场景至少 3 |
