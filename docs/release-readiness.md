...
- 2026-09-19 补充：DeepSeek 摘要流取消已在受控本机 relay 窗口通过，首个非空内容到达后取消、无摘要落盘及后续对话均验证；本地回归 10/10、验收报告测试 23/23 通过。此项不代替未经延迟的网络时序或原生 TUI，完整 compact 仍需应用队列/扩展等对照。

## 当前尚未满足

2026-09-20：手机已增加服务管理目录内未映射历史的发现、确认认领与续聊入口；导入不重放旧命令，真实进程子集验证原上下文延续。当前不回填旧 JSONL 到手机时间线，Android/iOS 真机找回流程仍未验收；终端专用 custom UI 仍有适配工作。

2026-09-19 默认配置补充：已修复新会话快照未发布实际模型、idle worker 确认停止后不退出的问题。DeepSeek 的默认值/已加载会话隔离、空历史回收恢复实测通过；扩展模型选择错误仅在两个合成模型下通过，真实单模型不触发该 hook。后端回归 13/13、进程故障 E2E 9/9 通过，首次真实断言失败仍保留且不推断根因。完整原生 TUI 和双模型 provider 验收仍未完成，详见[开发进度](progress.md)。
