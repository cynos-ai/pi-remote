...
状态：`blocked`。S12 Docker 部署子集已经在 WSL 2 通过生命周期验证，仍需在当前源码完成部署与恢复复验；S13 只在所有范围内必需阶段报告、真实 provider 常用流程和 Android / iOS 实机证据齐全后才能改为 `passed`。完整原生 pi TUI 复刻不属于 V1 发布门槛。本记录不触发任何外部发布、镜像推送或应用商店操作。
...
- 2026-09-19 修复 compact 保留原生队列时旧 Run 被强制记为 aborted 的问题。DeepSeek 的原生队列/摘要/后续文件对照与摘要生成前取消对照分别通过；该轮本地后端回归 8/8、验收报告测试 22/22 通过，详见[开发进度](progress.md)。

- 2026-09-19 补充：DeepSeek 摘要流取消已在受控本机 relay 窗口通过，首个非空内容到达后取消、无摘要落盘及后续对话均验证；本地回归 10/10、验收报告测试 23/23 通过。此项不代替未经延迟的网络时序，完整 compact 仍需应用队列/扩展等范围内对照。

## 当前尚未满足

2026-09-20：手机已增加服务管理目录内未映射历史的发现、确认认领与续聊入口；导入不重放旧命令，真实进程子集验证原上下文延续。当前不回填旧 JSONL 到手机时间线，Android/iOS 真机找回流程仍未验收。

2026-09-22：V1 范围收敛为常用移动流程。终端像素排版、跨 custom 实例共享焦点、聊天区域清屏/重绘、完整主题、硬件光标、全屏/滚动区和所有低频 TUI 快捷键明确为非目标，不再以 `tui.terminal-components: needs_adapter` 阻塞发布。扩展的常用输出和交互继续通过文本投影、触控操作和标准表单承载。

2026-09-19 默认配置补充：已修复新会话快照未发布实际模型、idle worker 确认停止后不退出的问题。DeepSeek 的默认值/已加载会话隔离、空历史回收恢复实测通过；扩展模型选择错误仅在两个合成模型下通过，真实单模型不触发该 hook。后端回归 13/13、进程故障 E2E 9/9 通过，首次真实断言失败仍保留且不推断根因。双模型 provider 验收仍未完成，详见[开发进度](progress.md)。
...
2. 在同一 Linux 环境完成 `pnpm test:bash-parity` 和 `pnpm test:tui-parity` 的常用行为基线，保存清洗后的报告；不采集终端像素或完整 TUI 模拟证据。
