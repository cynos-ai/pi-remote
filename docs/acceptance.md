...
editor 工厂补充 AT21 / AT22 / AT32：原工厂 getter、原生主题/快捷键/补全、草稿保留、光标粘贴、重复同步不重置光标、onSubmit 原值、正常/失败提交、替换后旧回调隔离、安装异常 dispose。真实 CustomEditor + HTTPS/WSS 验证 Tab 补全、普通模型提交一次、扩展 slash、`!!` 排除上下文、终端菜单不误投模型、清除保留草稿及失效旧按键拒绝。应用动作另验证空闲/活动 follow-up、混合持久输入的 dequeue、完整 editor 表单回填、思考显示设置/重放及失败草稿恢复。图片剪贴板、进程挂起、原生 TUI 与设备键盘独立验收。
...
分享补充（AT21 / AT22 / AT32）：目标选择和登录检查先于预览；未确认、关闭编辑器和失效旧表单均不得上传。HTML/JSONL 原文可完整翻页，确认显示目标、范围、字节数及 SHA-256；上传字节必须与预览一致，预览后会话变化不改变副本。重复表单响应最多触发一次本次调用；上传失败/回复丢失/重启不自动重试，不声称远端未创建。Radius 保留原生分支与 pi.share 元数据、organization 可见性，Gist 保留 Secret Gist HTML；不隐式降级或切换目标。定向合成 transport 与本地假 gh 的真实进程测试不等于真实账号上传、TUI 或设备验收。

## 编辑器导入、克隆和重载补充验收（AT02 / AT12 / AT19 / AT32）

克隆包含当前叶节点，源历史不变，不自动生成 prompt；导入确认可取消，完整复制后 ID/cwd 映射一致，重复导入同 ID 更新路径而不创建重复应用 Session，不重放旧输入。损坏/缺失历史明确失败。缺失 cwd 显示原路径并要求显式提交同 owner 已注册项目：取消、未注册目录与 intent 拒绝保持当前 runtime、草稿和源字节；成功副本 header 为目标真实路径，重启续聊保留上下文。原 cwd 尚存时拒绝 override。bound 前 SIGKILL 不重放未知导入，修订副本可经历史找回认领；源文件保持原 cwd 和字节。重载清理旧 UI、监听和页脚，新资源命令/键位可用，session_start 表单正常待答；生成中按原生条件拒绝重载，不停止 Run。各项合成/进程证据与真实 provider、TUI、设备验收分别记录。
