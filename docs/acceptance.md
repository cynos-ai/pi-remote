...
| AT25 | FR10, FR11 | S06, S12 | linux + docker | 正常 TERM / abort 清理长 Bash 及子进程；SIGKILL worker 时 Bash 独立 PGID 可存活，必须 blocked 且无第二 writer，同 scope 的 Node 重启不解锁。S06 验证逻辑、实例锁与 PID 复用；S12 验证第二容器新 scope / 错误容器证明拒绝、正确 stop / wait / inspect 才解清理阻塞，队列仍暂停；stop 超时 / 半写证据 / helper 崩溃保持阻塞 |
