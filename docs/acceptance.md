...
鉴权菜单补充（AT02 / AT32）：`/logout` 验证元数据列表、原生搜索/取消、重复菜单与旧响应失效、真实 auth.json 删除、可用模型变化、配置和活动请求保留、读取/写入异常及删除成功后同步失败的准确报告。合成凭据不得出现在会话事件或命令记录；真实 provider/OAuth 与设备另行验收。`/login` 的非持久化秘密通道尚待实现，普通表单通过不构成登录验收。
